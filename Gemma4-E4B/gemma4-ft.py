...
# Work around TorchDynamo fullgraph recompile-limit failures on some nightly builds.
# Set DISABLE_TORCH_DYNAMO=0 in .env to re-enable Dynamo if your environment is stable.
if os.getenv("DISABLE_TORCH_DYNAMO", "1") == "1":
...
class TrainingMetricsCallback(TrainerCallback):
    ...
    def _extract_json_candidate(self, text):
        ...
        # Gemma can emit thought/final channel wrappers around the answer.
        text = re.sub(r"<\|channel\|>thought\s*.*?<channel\|>", "", text, flags=re.DOTALL)
        text = text.replace("<|channel|>final", "").replace("<channel|>", "").strip()
    ...
    def _find_first_valid_json_object(self, text):
        candidate = str(text)
        depth = 0
        start_idx = -1
            ...
            if ch == "{":
                ...
                if depth > 0:
                    ...
                    if depth == 0 and start_idx != -1:
                        fragment = candidate[start_idx:idx + 1]
                        try:
    ...
    def _safe_load_json(self, text):
        try:
            ...
            return self._find_first_valid_json_object(self._extract_json_candidate(text))
    ...
    def _prepare_generation_inputs(self, sample, model):
        ...
        image_input = self._extract_image_from_sample(sample)
        ...
        if image_input is not None:
            proc_kwargs["images"] = [image_input]
    ...
    def _compute_research_metrics(self, model):
        if model is None or not self._eval_samples:
        ...
        if processor.tokenizer.eos_token_id is not None:
            stop_tokens.append(processor.tokenizer.eos_token_id)
        ...
        if pad_token_id is None and processor.tokenizer.eos_token_id is not None:
        ...
        if sample_count == 0:
            print(
                "⚠ Research metrics were computed on 0 samples "
                f"(target_missing={target_missing}, exceptions={exception_count}, eval_pool={len(self._eval_samples)})."
            )
            return self._empty_research_metrics()
...
def plot_training_metrics(metrics: dict, step_losses: list, output_dir: str):
    ...
    for ax, (key, title, color, ylabel, is_pct) in zip(axes[1:], panel_cfg):
        ...
        if not values:
            continue
        ax.plot(epochs, values, color=color, linewidth=2, marker="o", markersize=5, zorder=3)
        ...
        best_idx = values.index(max(values)) if is_pct else values.index(min(values))
        ax.annotate(
